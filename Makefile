...
all: unit-tests d2ctl image-d2ctl lint
...
fmt:  ## Formats the source code
	@docker run --rm -it -v $(PWD):/src -w /src golang:$(GO_VERSION) \
		...
		gofumports -w -local github.com/talos-systems/day-two ."
...
.PHONY: $(ARTIFACTS)/d2ctl-linux-amd64
$(ARTIFACTS)/d2ctl-linux-amd64:
	@$(MAKE) local-d2ctl-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: d2ctl-linux-amd64
d2ctl-linux-amd64: $(ARTIFACTS)/d2ctl-linux-amd64  ## Builds executable for d2ctl-linux-amd64.

.PHONY: d2ctl
d2ctl: d2ctl-linux-amd64  ## Builds executables for d2ctl.
...
.PHONY: image-d2ctl
image-d2ctl:  ## Builds image for d2ctl.
	@$(MAKE) target-$@ TARGET_ARGS="--tag=$(REGISTRY)/$(USERNAME)/d2ctl:$(TAG)"
